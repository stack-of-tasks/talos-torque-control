...
const double DEFAULT_MAX_CURRENT = 5;     /// max CURRENT (double in [0 Amp, 20 Amp]) 
      ...
      const double RIGHT_FOOT_FORCE_SENSOR_XYZ[3] = {0.0,   0.0, -0.0}; // not updated for talos/talos
      const double LEFT_FOOT_FORCE_SENSOR_XYZ[3]  = {0.0,   0.0, -0.0}; // not updated for talos/talos
      ...
      const double RIGHT_HAND_FORCE_SENSOR_Z_ROTATION = -0.5*M_PI;// not updated for talos/talos
      const double LEFT_HAND_FORCE_SENSOR_Z_ROTATION  = -0.5*M_PI;// not updated for talos/talos
      ...
      const double RIGHT_FOOT_FORCE_SENSOR_MASS_PERCENTAGE = 0.65; // not updated for talos/talos
      const double LEFT_FOOT_FORCE_SENSOR_MASS_PERCENTAGE  = 0.65; // not updated for talos/talos
      const double RIGHT_HAND_FORCE_SENSOR_MASS_PERCENTAGE = 0.75; // not updated for talos/talos
      const double LEFT_HAND_FORCE_SENSOR_MASS_PERCENTAGE  = 0.75; // not updated for talos/talos
      ...
      struct JointUtil
      {
        static std::map<unsigned int,JointLimits> create_id_2_limits_map()
        {
          ...
          m[22]  = JointLimits(-0.523598775598, 1.57079632679);     // right arm 1 shoulder
